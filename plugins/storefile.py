...
async def storefile(c, m):
    ...
    if not m.photo:
        ...
        text += (
            f"📊 __File Size:__ `{humanbytes(media.file_size)}`\n\n"
            if media.file_size
            else ""
        )
        if not m.document:
            text += (
                f"🎞 __Duration:__ `{TimeFormatter(media.duration * 1000)}`\n\n"
                if media.duration
                else ""
            )
            if m.audio:
                ...
                text += (
                    f"🎙 __Performer:__ `{media.performer}`\n\n"
                    if media.performer
                    else ""
                )
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="Delete 🗑", callback_data=f"delete+{msg.id}"
            )
        ],
    ]

    # sending message
    await send_message.edit(text, reply_markup=InlineKeyboardMarkup(buttons))
...
async def storefile_channel(c, m):
    ...
    text += (
        f"__👁 Members Count:__ {m.chat.members_count}\n\n"
        if m.chat.members_count
        else ""
    )
