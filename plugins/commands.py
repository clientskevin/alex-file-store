    ...
    if len(m.command) > 1:  # sending the stored file
        ...
        if as_uploadername:
            if chat_id.startswith("-100"):
                ...
                caption += (
                    f"__🗣 User Name:__ @{channel.username}\n\n"
                    if channel.username
                    else ""
                )
                ...
                caption += (
                    f"__👁 Members Count:__ {channel.members_count}\n\n"
                ...
                caption += (
                    f"__🐧 Last Name:__ `{user.last_name}`\n\n"
                    if user.last_name
                    else ""
                )
                caption += (
                    f"__👁 User Name:__ @{user.username}\n\n" if user.username else ""
                )
...
async def me(c, m):
    ...
    text += (
        f"__✔ Is Verified By TELEGRAM:__ `{me.is_verified}`\n\n"
        if me.is_verified
        else ""
    )
    ...
    while m.from_user.id in BATCH:
        if i == 1:
            media = await c.ask(
                ...
                text="Send me some files or videos or photos or text or audio. If you want to cancel the process send /cancel",
            )
            ...
            try:
                ...
                media = await c.ask(
                    chat_id=m.from_user.id,
                    text="Ok 😉. Now send me some more files Or press done to get shareable link. If you want to cancel the process send /cancel",
